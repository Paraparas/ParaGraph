...
import { refineRawTranscriptPrompt, timelineViewPrompt, quickViewPrompt, topicViewPrompt } from './prompts';
import { llmConfig } from '../../config/llm';
import { writeFile, mkdir } from 'fs/promises';
import path from 'path';
import { QuickViewData } from '../types/quickView';
...
export class TranscriptProcessor {
  ...
  // async generateQuickViewData(refinedTranscript: string): Promise<ProcessingResponse> {
  //   try {
  //     this.processingStages = [];
  //     this.updateStage('parsing', 'processing');
  //     console.log('Current NODE_ENV:', process.env.NODE_ENV);
  ...
  //       model: this.config.model,
  //       messages: [
  //         { role: 'system', content: quickViewPrompt },
  //         { role: 'user', content: refinedTranscript }
  //       ],
  //       temperature: this.config.temperature,
  //       max_tokens: this.config.maxTokens,
  ...
  //     if (process.env.NODE_ENV === 'development') {
  //       try {
  //         const outputPath = path.join(process.cwd(), 'data', 'processed', 'quickViewProcessed.json');
  //         await mkdir(path.join(process.cwd(), 'data', 'processed'), { recursive: true });
  //         await writeFile(
  //           outputPath,
  ...
  //           'utf-8'
  //         );
  //         console.log('Quick view Processed data saved to:', outputPath);
  //       } catch (error) {
  //         console.error('Error saving quick view processed data:', error);
  //       }
  //     }
  ...
  //     return {
  //       success: true,
  //       data: parsedResponse as QuickViewData,
  //       status: 'completed',
  //       processingTime: Date.now() - (this.processingStages[0]?.startTime || Date.now())
  //     };

  //   } catch (error) {
  //     const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred while processing quickview data';
  //     this.updateStage('parsing', 'failed', errorMessage);
